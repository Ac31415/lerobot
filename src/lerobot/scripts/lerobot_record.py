...
def record_loop(
    ...
    while timestamp < control_time_s:
        ...
        if policy is not None and preprocessor is not None and postprocessor is not None:
            ...
            no_action_count += 1
            if dataset is not None and (no_action_count == 1 or no_action_count % 10 == 0):
                logging.warning(
            ...
            # Keep reset loops (where dataset is None) time-bounded and fps-paced
            # even when actions are intentionally disabled.
        ...
        with VideoEncodingManager(dataset):
            ...
            while recorded_episodes < cfg.dataset.num_episodes and not events["stop_recording"]:
                ...
                if not events["stop_recording"] and (
                    ...
                    # Temporarily disconnect teleop during reset so no control
                    # state (e.g. TF buffers, reference captures) leaks across
                    # episode boundaries.
                    teleop_reconnect_after_reset = False
                    if teleop is not None and teleop.is_connected:
                        teleop.disconnect()
                        teleop_reconnect_after_reset = True
                    ...
                    if (
                        teleop_reconnect_after_reset
                        and not events["stop_recording"]
                        and teleop is not None
                        and not teleop.is_connected
                    ):
                        teleop.connect()
